...
if uploaded_file:
    ...
    # ggf. invertieren (falls weiße Strukturen auf schwarzem Hintergrund nötig)
    if np.mean(gray[mask == 255]) > np.mean(gray[mask == 0]):
        mask = cv2.bitwise_not(mask)

    st.image(mask, caption="Maske", use_container_width=True)

    # -------------------- 3. Flecken direkt in Maske zählen --------------------
    ...
    # Ergebnis direkt auf Maske zeichnen
    mask_colored = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    for (x, y) in centers:
        cv2.circle(mask_colored, (x, y), 8, (0, 0, 255), 2)

    st.image(mask_colored, caption=f"Gefundene Strukturen: {len(centers)}", use_container_width=True)
